	...
	var sliderPoints = $( "<div id='slider-futpoints'></div>" ).insertAfter( selectPoints ).slider({
		...
		change: function(event, ui) { 
				...
				$('#futpoints-amount').html(sliderValuePoints);
					...
					else if (sliderValuePoints == '1500') {
...
4
	...
	$('#increase-futpoints').click(function() {
		...
		sliderPoints.slider( "value", sliderCurrentPointsValue + 60 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				setTimeout(function() { 
					$console_message.html("Collecting Information");	
					progressBarConsole(84, $('#progressBarConsole'));			
				}, 25900 );
				setTimeout(function() { 
					$console_message.html("Information Collected");	
					progressBarConsole(90, $('#progressBarConsole'));			
				}, 27000 );
				setTimeout(function() { 
					$console_message.html("Performing Order");	
					progressBarConsole(93, $('#progressBarConsole'));			
				}, 28000 );
				setTimeout(function() { 
					$console_message.html("<span class='console-message-error'>Automatic UC BP Sending Failed</span>");	
					progressBarConsole(93, $('#progressBarConsole'));			
				}, 30000 );
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>Manual Sending PUBG UC BP Required</span>");
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
					progressBarConsole(93, $('#progressBarConsole'));			
				...
				setTimeout(function() {
					...
					$console_message.html("Please Click On The Button & Get Your Free PUBG UC BP");	
					if ($(window).width() < 600) {
